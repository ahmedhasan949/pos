...
class User_lib
{
    ...
    public function Login_user($request = array())  {   
        ...
        if (empty($request['username']) && empty($request['password']) ) {
            ...
            $response['result']['response'] = "Username Or Password must not be empty";
            return array($response, 406);
        } 
        
        if(empty($request['role'])) {
            $response['result']['status'] = 'error';
            $response['result']['response'] = "Role Must not be Empty";
            return array($response, 406);
        }
        
		
        $get_user_auth = $this->CI->User_model->UserAuthLogin($request); 
        
        if (!$get_user_auth) {
           
            $response['result']['status'] = 'error';
            $response['result']['response'] = "Invalid Username or Password";
            return array($response, 406);
        } 

        $response['result']['status'] = 'success';
        $response['result']['response'] = "User Details Successfully Recieved";
        $response['result']['data'] = $get_user_auth;
